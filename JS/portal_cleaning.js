...
$( "#approveBtn" ).click(function(e) {
    ...
    $( "#declineBtn" ).click(function(e) {
        ...
        if (document.getElementById("comment-on-request").value) {
            ...
            //if (document.title.indexOf("IP Form") !== -1) {
            if (document.title.indexOf("IP Form") !== -1) {
                var comment = prompt("При отклонении заявки, необходимо обязательно оставить комментарий (причина отклонения):", "");
                if (comment == null || comment == "") {
                    //txt = "User cancelled the prompt.";
                } else {
                    document.getElementById("comment-on-request").value= comment;
                    $( "#addBtn" ).trigger( "click" );
                    setTimeout(function() {
                        $("#declineBtn").off("click");
                        $("#declineBtn").trigger("click");
                ...
                //alert("При отклонении IP Form, необходимо заполнить поле комментария, но не отправлять его");
            } else {
                $("#declineBtn").off("click");
                $("#declineBtn").trigger("click");
            }
